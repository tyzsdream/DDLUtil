...
public class Column implements Serializable {
    ...
    public String getDefaultValue() {
        if ((_defaultValue != null) && (_defaultValue.length() > 0)&&!"now".equalsIgnoreCase(_defaultValue)&& (_typeCode == Types.DATE || _typeCode == Types.TIME || _typeCode == Types.TIMESTAMP)) {
            try {
                switch (_typeCode) {
                    case Types.DATE:
                        Date.valueOf(_defaultValue);
                        break;
                    case Types.TIME:
                        Time.valueOf(_defaultValue);
                        break;
                    case Types.TIMESTAMP:
                        Timestamp.valueOf(_defaultValue);
                        break;
                }
            } catch (Exception e) {
                _defaultValue = "now";
    ...
    public Object getParsedDefaultValue() {
        if ((_defaultValue != null) && (_defaultValue.length() > 0)) {
            try {
                ...
                // TODO TYZ 解决系统默认时间问题
                if (_typeCode == Types.DATE || _typeCode == Types.TIME || _typeCode == Types.TIMESTAMP) {
                    _defaultValue = "now";
                    return "now";
                }
                return null;
