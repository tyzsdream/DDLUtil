...
public abstract class PlatformImplBase extends JdbcSupport implements Platform {
    ...
    public void createDatabase(String jdbcDriverClassName, String connectionUrl, String username, String password,
                               Map<String, String> parameters) throws DatabaseOperationException, UnsupportedOperationException {
    ...
    protected List<ModelChange> sortChanges(List<ModelChange> changes) {
        ...
        Collections.sort(changes, new Comparator<ModelChange>() {
     ...
     * @param model   The database model; this object is not going to be changed by this
     *                method
    ...
    protected Database processChanges(Database model, Collection<ModelChange> changes, CreationParameters params)
        ...
        for (Iterator<ModelChange> it = changes.iterator(); it.hasNext(); ) {
        ...
        for (Iterator<TableChange> it = change.getOriginalChanges().iterator(); canMigrateData && it.hasNext(); ) {
            ...
            if (curChange instanceof AddColumnChange) {
                ...
                if (addColumnChange.getNewColumn().isRequired() && !addColumnChange.getNewColumn().isAutoIncrement()
                    ...
                    //todo tyz 添加了非空字段且没有默认值如何处理：暂时报错
                    throw new DdlUtilsException("Data cannot be retained in table " + change.getChangedTable()
...
//                    canMigrateData = false;
        ...
        Map<String, String> parameters = (params == null ? null : params.getParametersFor(targetTable));
...
//            getSqlBuilder().createTable(currentModel, targetTable, parameters);
     ...
     * {@inheritDoc}
     */
    public Iterator<DynaBean> query(Database model, String sql, Collection<Object> parameters, Table[] queryHints)
            throws DatabaseOperationException {
        Connection connection = borrowConnection();
     ...
     * {@inheritDoc}
     */
    public List<DynaBean> fetch(Database model, String sql, Table[] queryHints, int start, int end)
            throws DatabaseOperationException {
        Connection connection = borrowConnection();
        Statement statement = null;
        ResultSet resultSet = null;
     ...
     * {@inheritDoc}
     */
    public List<DynaBean> fetch(Database model, String sql, Collection<Object> parameters, int start, int end)
            throws DatabaseOperationException {
    ...
    private SqlDynaProperty[] getPropertiesForInsertion(Database model, SqlDynaClass dynaClass, final DynaBean bean) {
        ...
        Collection<SqlDynaProperty> result = CollectionUtils.select(Arrays.asList(properties), new Predicate() {
    ...
    private Column[] getRelevantIdentityColumns(Database model, SqlDynaClass dynaClass, final DynaBean bean) {
        ...
        @SuppressWarnings("unchecked")
        Collection<SqlDynaProperty> relevantProperties = CollectionUtils.select(Arrays.asList(properties),
                new Predicate() {
                    public boolean evaluate(Object input) {
                        SqlDynaProperty prop = (SqlDynaProperty) input;
     ...
     * {@inheritDoc}
     */
    public void insert(Connection connection, Database model, Collection<DynaBean> dynaBeans)
            throws DatabaseOperationException {
        ...
        for (Iterator<DynaBean> it = dynaBeans.iterator(); it.hasNext(); ) {
     ...
     * @param primaryKeys The primary keys
     * @param properties  The properties to write
     * @param oldBean     Contains column values to identify the rows to update (i.e. for
     *                    the WHERE clause)
     ...
     * @param property  The property of the bean, which also defines the corresponding
     *                  column
     */
     ...
     * retrieved from the result set is returned.<br/>
     * The method is defined here rather than in the
     ...
     * Its primary purpose is to call the appropriate method on the result set, and
     ...
     * @param queryHints The tables that were queried in the query that produced the given
     *                   result set (optional)
     * @return The iterator
     */
